...
- Confidence flags spoken naturally — vary the actual wording episode to episode, e.g.
  ...
  two example phrasings just given illustrate the pattern; they are not text to paste
  into the script (added 2026-08-27 — an earlier version of this rule gave one fixed
  example phrase and it started showing up copy-pasted almost verbatim episode after
  episode, see RUNBOOK.md's Process log).
- Never say "great question", "absolutely", "let's dive in", "game-changer".
- Vocabulary variety (added 2026-08-27): don't lean on the same connective tissue every
  episode — words/phrases like "quietly", "genuinely", "honestly", "plainly", "worth
  noting", "here's the thing" are easy crutches and get noticeable fast at one episode a
...
Keep this order and this rough word allocation every episode, but vary the actual
phrasing used to move between sections and to sign off (added 2026-08-27). Don't say
"Part one — What shipped" or close with "Tomorrow, same time, same feed" the identical
way every single time — the structure staying fixed is good for a daily listener who
wants to know where they are; the wording staying fixed is what makes it feel like a
template being filled in rather than a person talking.

## Hard content rules
- Every news claim dated 2026 with a source the research pass actually read.
  Nothing from training data dressed up as news. Discard rumors.
- Never invent revenue numbers. Quote only published figures, attributed ("an
  interview on Indie Hackers", "a Show HN thread").
- Check archive/covered.md before writing: never re-cover an item; follow-ups are
  encouraged and should reference the earlier episode ("on Monday we covered X").
...
- Write for the ear: numbers as words where natural ("two and a half thousand dollars
  a month"), "per cent" not "%", "pounds" not "£".
